...
const RuleMatchQuery string = `with rulematch as (select rs.ruleset, t.tablematchnum, rs.rulenum, t.reloid, t.relnamespace, t.relname, t.reltuples, t.relkind from pg_temp.tables t join pg_temp.rulesets rs on t.ruleset = rs.ruleset and case
	  when t.reltuples >= rs.minrows then 't'::bool
	  else 'f'::bool end),
	effective_settings_sub1 as (select rm.tablematchnum, rm.rulenum, rm.reloid, rm.relnamespace, rm.relname, rm.reltuples, rm.relkind, rss.parameter, rss.setting from rulematch rm join pg_temp.rulesets_settings rss on rm.ruleset = rss.ruleset and rm.rulenum=rss.rulenum),
	effective_settings_sub2 as (select reloid, relnamespace, relname, reltuples, relkind, tablematchnum, parameter, setting from effective_settings_sub1 where (tablematchnum, rulenum, reloid, relnamespace, relname, parameter) in (select tablematchnum, max(rulenum) as rulenum, reloid, relnamespace, relname, parameter from effective_settings_sub1 group by tablematchnum, reloid, relnamespace, relname, parameter)),
	effective_settings as (select ess.reloid, ess.relnamespace, ess.relname, ess.reltuples, ess.relkind, ess.tablematchnum, ess.parameter, topt.setting as oldsetting, ess.setting as newsetting from effective_settings_sub2 ess left outer join tableoptions topt on ess.reloid=topt.reloid and ess.parameter=topt.parameter where (ess.setting is null and (ess.reloid, ess.parameter) in (select reloid, parameter from tableoptions)) or (ess.setting is not null and (ess.reloid, ess.parameter, ess.setting) not in (select reloid, parameter, setting from tableoptions)))
	select reloid::integer, relkind, format('%I.%I',relnamespace,relname) as quotedfullname, reltuples, jsonout, tablematchnum from (select reloid, relnamespace, relname, reltuples, relkind, tablematchnum, json_object_agg(parameter, json_build_object('oldsetting',oldsetting,'newsetting',newsetting)) as jsonout from effective_settings group by reloid, relnamespace, relname, reltuples, relkind, tablematchnum order by relnamespace, relname) sub`
