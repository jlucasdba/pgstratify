...
rulesetsjsonin as (select $2::jsonb as rulesetsjsonin),
...
tableoptions as (select reloid, reloptions[1] as option, reloptions[2] as setting from (select oid as reloid, regexp_split_to_array(unnest(reloptions),'=') as reloptions from pg_class where oid in (select reloid from tables)) tableoptions_a),
rulematch as (select rs.ruleset, t.tablematchnum, rs.rulenum, t.reloid, t.relnamespace, t.relname from tables t join rulesets rs on t.ruleset = rs.ruleset and case
  when rs.operator = 'ge' and t.reltuples >= rs.threshold then 't'::bool
  when rs.operator = 'lt' and t.reltuples < rs.threshold then 't'::bool
  else 'f'::bool end),
effective_settings_sub1 as (select rm.tablematchnum, rm.rulenum, rm.reloid, rm.relnamespace, rm.relname, rss.option, rss.setting from rulematch rm join rulesets_settings rss on rm.ruleset = rss.ruleset and rm.rulenum=rss.rulenum),
